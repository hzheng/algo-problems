...
public class WordBreak2 {
    ...
    public List<String> wordBreak4(String s, Set<String> wordDict) {
        return wordBreak4(s, wordDict, new HashMap<>());
    }

    private List<String> wordBreak4(String s, Set<String> wordDict,
        ...
        for (int i = 1, len = s.length(); i <= len; i++) {
            ...
            if (wordDict.contains(left)) {
                for (String word : wordBreak4(s.substring(i), wordDict, map)) {
                    res.add(left + " " + word);
                }
                if (i == len) {
                    res.add(left);
