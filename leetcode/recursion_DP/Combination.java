...
public class Combination {
    ...
    private void combine(int n, int k, List<List<Integer>> res, List<Integer> cur) {
        int count = cur.size();
        if (count == k) {
    ...
    // Solution of Choice
    // Recursion + DFS + Backtracking
    // 19 ms(67.08%), 40.5 MB(66.28%) for 27 tests
    ...
    private void combine1(int n, int k, int start, List<List<Integer>> res, List<Integer> buf) {
        int count = buf.size();
        if (count == k) {
    ...
    // Solution of Choice
    // Recursion + DFS + Backtracking
    // 22 ms(50.14%), 40.9 MB(57.36%) for 27 tests
    public List<List<Integer>> combine1_1(int n, int k) {
        List<List<Integer>> res = new ArrayList<>();
        dfs(n, k, 1, new ArrayList<>(), res);
        return res;
    }

    private void dfs(int n, int k, int i, List<Integer> buf, List<List<Integer>> res) {
        if (buf.size() == k) {
            res.add(new ArrayList<>(buf));
            return;
        }
    ...
    void test(Function<Integer, Integer, List<List<Integer>>> combine, String name, int n, int k,
              Integer[][] expected) {
    ...
    void test(int n, int k, Integer[][] expected) {
        ...
        test(c::combine1_1, "combine1_1", n, k, expected);
    ...
    @Test public void test1() {
        test(4, 3, new Integer[][] {{1, 2, 3}, {1, 2, 4}, {1, 3, 4}, {2, 3, 4}});
        test(5, 3,
             new Integer[][] {{1, 2, 3}, {1, 2, 4}, {1, 2, 5}, {1, 3, 4}, {1, 3, 5}, {1, 4, 5},
