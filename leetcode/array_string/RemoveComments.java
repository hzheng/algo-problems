...
// LC722: https://leetcode.com/problems/remove-comments/
//
// Given a C++ program, remove comments from it.
public class RemoveComments {
    // Recursive
    // beats 31.26%(6 ms for 53 tests)
    public List<String> removeComments(String[] source) {
        ...
        for (String line : source) {
            ...
            if (!inComment) {
                inComment = check(line, 0, buffer);
                if (inComment) {
                    uncommented.append(buffer);
                } else if (buffer.length() > 0) {
                    res.add(buffer.toString());
                }
            } else {
                int bcEnd = line.indexOf("*/");
                if (bcEnd >= 0) {
                    inComment = check(line, bcEnd + 2, buffer);
                    uncommented.append(buffer);
                    if (!inComment && uncommented.length() > 0) {
                        res.add(uncommented.toString());
                        uncommented.setLength(0);
                    }
                }
            }
        }
    ...
    private boolean check(String line, int start, StringBuilder sb) {
        int lcStart = line.indexOf("//", start);
        int bcStart = line.indexOf("/*", start);
        if (lcStart < 0 && bcStart < 0) {
            sb.append(line.substring(start));
            return false;
        }

        if (bcStart < 0 || lcStart >= 0 && bcStart > lcStart) {
            if (lcStart > 0) {
                sb.append(line.substring(start, lcStart));
            }
            return false;
        }

        sb.append(line.substring(start, bcStart));
        int bcEnd = line.indexOf("*/", bcStart + 2);
        return (bcEnd < 0) ? true : check(line, bcEnd + 2, sb);
    }

    // beats 70.40%(5 ms for 53 tests)
    public List<String> removeComments2(String[] source) {
        ...
        for (String line : source) {
            ...
            for (int i = 0, len = cs.length; i < len; i++) {
                if (inBlockComment) {
                    if (i + 1 < len && cs[i] == '*' && cs[i + 1] == '/') {
                        inBlockComment = false;
                        i++;
                    }
                } else if (i + 1 < len && cs[i] == '/' && cs[i + 1] == '*') {
                    inBlockComment = true;
                    i++;
                } else if (i + 1 < len && cs[i] == '/' && cs[i + 1] == '/') {
                    break;
    ...
    void test(String[] source, String[] expected) {
        assertEquals(Arrays.asList(expected), removeComments(source));
        assertEquals(Arrays.asList(expected), removeComments2(source));
        assertEquals(Arrays.asList(expected), removeComments3(source));
    }

    @Test
    public void test() {
        test(new String[] {"a/*", "*/", "b/*", "*/"}, new String[] {"a", "b"});
        test(new String[] {"class test{", "public: ", "   int x = 1;",
                           "   /*double y = 1;*/", "   char c;", "};"},
             new String[] {"class test{", "public: ", "   int x = 1;", "   ",
                           "   char c;", "};"});
        test(new String[] {"/*Test program */", "int main()", "{ ",
                           "  // variable declaration ",
                           "int a, b, c;", "/* This is a test",
                           "   multiline  ",
                           "   comment for ", "   testing */", "a = b + c;",
                           "}"},
             new String[] {"int main()", "{ ", "  ", "int a, b, c;",
                           "a = b + c;", "}"});
        test(new String[] {"a/*/b//*c", "blank", "d/*/e*//f"},
             new String[] {"ae*"});
        test(new String[] {"void func(int k) {",
                           "// this function does nothing /*", "   k = k*2/4;",
                           "   k = k/2;*/", "}"},
             new String[] {"void func(int k) {", "   k = k*2/4;",
                           "   k = k/2;*/", "}"});
        test(new String[] {"a/*comment", "line", "more_comment*/b"},
             new String[] {"ab"});
        test(
            ...
            new String[] {"aec*", "ec", "ddadbede", "e", "eed*", "bab", "c/bb*", "cbae*", "dcabebdc", "badcc",
                          "dd*", "eb/dcdbaaadd", "ba*", "ab", "*", "*", "aeabdccccd", "c/aa", "de/aedb*", "*",
                          "*", "/dc", "e/edceacc/ea*", "ca", "ec", "ebdce", "dadc", "eadddaabebeedd",
                          "cbeadebcaebded*", "ee", "eb", "dd", "cbccc", "da*", "d*", "b*", "dac*", "de",
                          "e", "b", "dbbbe", "ccd*", "*", "adaabdaaea", "eec*", "/a/addc", "*", "*",
                          "/ddddcab*", "cb*", "b*", "*", "aaadddd", "bd*", "ad", "*", "*", "e", "a*", "a",
                          "d*", "e*", "cedc*", "*", "*", "eb", "*", "b*", "*", "ba*", "da*", "eccd*/ab",
                          "*", "*/cbcedae*", "a", "aa", "*", "cadbbd", "d", "*", "c", "d/d", "d/c",
                          "dbbdedece"});
    }
