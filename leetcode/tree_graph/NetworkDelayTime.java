...
public class NetworkDelayTime {
    ...
    // beats %(121 ms for 51 tests)
    public int networkDelayTime(int[][] times, int N, int K) {
        Map<Integer, List<int[]> > graph = new HashMap<>();
        for (int[] edge : times) {
            List<int[]> edges = graph.get(edge[0]);
            if (edges == null) {
                graph.put(edge[0], edges = new ArrayList<>());
            }
        ...
        while (!pq.isEmpty()) {
            int[] head = pq.poll();
            int target = head[0];
            if (timeMap.containsKey(target)) continue;

            int time = head[1];
            timeMap.put(target, time);
            List<int[]> edges = graph.get(target);
            if (edges == null) continue;

            for (int[] edge : edges) {
                if (!timeMap.containsKey(edge[0])) {
                    pq.offer(new int[] {edge[0], time + edge[1]});
                }
            }
        }
        if (timeMap.size() != N) return -1;
    ...
    public int networkDelayTime2(int[][] times, int N, int K) {
        ...
        while (true) {
            ...
            List<int[]> edges = graph.get(nearest);
            if (edges == null) continue;

            for (int[] edge : edges) {
                timeMap.put(edge[0],
                            Math.min(timeMap.get(edge[0]), time + edge[1]));
            }
        }
        int res = 0;
        for (int v : timeMap.values()) {
    ...
    // Sort + Hash Table + DFS + Recursion
    // time complexity: O(N ^ N + E * log(E)), space complexity: O(N + E)
    // beats %(204 ms for 51 tests)
    public int networkDelayTime3(int[][] times, int N, int K) {
        ...
        for (List<int[]> edges : graph.values()) {
            Collections.sort(edges, (a, b) -> a[1] - b[1]);
        }
        Map<Integer, Integer> timeMap = new HashMap<>();
        for (int i = 1; i <= N; i++) {
            timeMap.put(i, Integer.MAX_VALUE);
    ...
    private void dfs(Map<Integer, List<int[]> > graph, int v, int time,
                     Map<Integer, Integer> timeMap) {
        if (time >= timeMap.get(v)) return;
    ...
    public int networkDelayTime4(int[][] times, int N, int K) {
        ...
        for (queue.offer(K); !queue.isEmpty(); ) {
            ...
            for (int i = queue.size(); i > 0; i--) {
                int u = queue.poll();
                for (int v = 1; v <= N; v++) {
                    if (edges[u][v] != null && delay[u] + edges[u][v] <
                        delay[v]) {
                        if (!visited[v]) {
                            visited[v] = true;
                            queue.offer(v);
